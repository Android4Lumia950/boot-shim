...
BOOLEAN CheckElf32Header(Elf32_Ehdr* bl_elf_hdr) {
    ...
    return bl_elf_hdr->e_ident[EI_MAG0] == ELFMAG0 &&
           ...
           bl_elf_hdr->e_machine == EM_ARM &&
           bl_elf_hdr->e_type == ET_EXEC &&
           bl_elf_hdr->e_phnum >= 1;
}
...
EFI_STATUS efi_main(
    ...
    for (UINTN index = 0; index < NumHandles; ++index) {
        ...
        if (EFI_ERROR(Status) || PayloadFileInformation->FileSize > UINT32_MAX) goto local_cleanup;
        ...
        if (!CheckElf32Header(PayloadElf32Ehdr)) goto local_cleanup;
        ...
        for (UINTN ph_idx = 0; ph_idx < PayloadElf32Ehdr->e_phnum; ++ph_idx) {
            ...
            if (Phdr->p_type == PT_LOAD && Phdr->p_paddr == Phdr->p_vaddr &&
                Phdr->p_filesz == Phdr->p_memsz && Phdr->p_paddr == UefiEntryPoint) {
        ...
        if (PayloadSectionOffset == 0 || PayloadLength == 0) goto local_cleanup;
